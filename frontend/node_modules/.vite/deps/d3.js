...
function cumsum(values, valueof) {
  var sum3 = 0, index3 = 0;
  return Float64Array.from(values, valueof === void 0 ? (v2) => sum3 += +v2 || 0 : (v2) => sum3 += +valueof(v2, index3++, values) || 0);
}
...
function variance(values, valueof) {
  ...
  if (count3 > 1) return sum3 / (count3 - 1);
}
...
function bin() {
  ...
  function histogram(data) {
    ...
    if (isFinite(step)) {
      if (step > 0) {
        for (i = 0; i < n; ++i) {
          if ((x3 = values[i]) != null && x06 <= x3 && x3 <= x12) {
            bins[Math.min(m3, Math.floor((x3 - x06) / step))].push(data[i]);
          }
        }
      } else if (step < 0) {
        for (i = 0; i < n; ++i) {
          if ((x3 = values[i]) != null && x06 <= x3 && x3 <= x12) {
            const j = Math.floor((x06 - x3) * step);
            bins[Math.min(m3, j + (tz[j] <= x3))].push(data[i]);
          }
        }
      }
    } else {
      for (i = 0; i < n; ++i) {
        if ((x3 = values[i]) != null && x06 <= x3 && x3 <= x12) {
          bins[bisect_default(tz, x3, 0, m3)].push(data[i]);
        }
      }
    }
    return bins;
...
function quickselect(array3, k, left2 = 0, right2 = Infinity, compare) {
  ...
  while (right2 > left2) {
    if (right2 - left2 > 600) {
      ...
      const sd = 0.5 * Math.sqrt(z * s * (n - s) / n) * (m3 - n / 2 < 0 ? -1 : 1);
      const newLeft = Math.max(left2, Math.floor(k - m3 * s / n + sd));
      const newRight = Math.min(right2, Math.floor(k + (n - m3) * s / n + sd));
...
function thresholdFreedmanDiaconis(values, min3, max4) {
  const c5 = count(values), d = quantile(values, 0.75) - quantile(values, 0.25);
  return c5 && d ? Math.ceil((max4 - min3) / (2 * d * Math.pow(c5, -1 / 3))) : 1;
}

// node_modules/d3-array/src/threshold/scott.js
function thresholdScott(values, min3, max4) {
  const c5 = count(values), d = deviation(values);
  return c5 && d ? Math.ceil((max4 - min3) * Math.cbrt(c5) / (3.49 * d)) : 1;
}
...
function rank(values, valueof = ascending) {
  ...
  values.forEach((j, i) => {
    const c5 = compareIndex(j, k === void 0 ? j : k);
    if (c5 >= 0) {
      if (k === void 0 || c5 > 0) k = j, r = i;
      R[j] = r;
    } else {
      R[j] = NaN;
    }
...
function lch(l, c5, h, opacity) {
  return arguments.length === 1 ? hclConvert(l) : new Hcl(h, c5, l, opacity == null ? 1 : opacity);
}
...
function hue(a3, b) {
  var d = b - a3;
  return d ? linear(a3, d > 180 || d < -180 ? d - 360 * Math.round(d / 360) : d) : constant_default2(isNaN(a3) ? b : a3);
}
function gamma(y3) {
...
function numberArray_default(a3, b) {
  if (!b) b = [];
  var n = a3 ? Math.min(b.length, a3.length) : 0, c5 = b.slice(), i;
  return function(t) {
    for (i = 0; i < n; ++i) c5[i] = a3[i] * (1 - t) + b[i] * t;
    return c5;
  };
}
...
function genericArray(a3, b) {
  var nb = b ? b.length : 0, na = a3 ? Math.min(nb, a3.length) : 0, x3 = new Array(na), c5 = new Array(nb), i;
  for (i = 0; i < na; ++i) x3[i] = value_default(a3[i], b[i]);
  for (; i < nb; ++i) c5[i] = b[i];
  return function(t) {
    for (i = 0; i < na; ++i) c5[i] = x3[i](t);
    return c5;
  };
}
...
function hue_default(a3, b) {
  ...
  return function(t) {
    var x3 = i(t);
    return x3 - 360 * Math.floor(x3 / 360);
  };
}
...
function decompose_default(a3, b, c5, d, e, f) {
  ...
  if (skewX = a3 * c5 + b * d) c5 -= a3 * skewX, d -= b * skewX;
  if (scaleY = Math.sqrt(c5 * c5 + d * d)) c5 /= scaleY, d /= scaleY, skewX /= scaleY;
  if (a3 * d < b * c5) a3 = -a3, b = -b, skewX = -skewX, scaleX = -scaleX;
  return {
    translateX: e,
    translateY: f,
...
function interpolateTransform(parse, pxComma, pxParen, degParen) {
  ...
  return function(a3, b) {
    ...
    scale2(a3.scaleX, a3.scaleY, b.scaleX, b.scaleY, s, q);
    a3 = b = null;
    return function(t) {
...
function hcl2(hue2) {
  return function(start2, end) {
    var h = hue2((start2 = hcl(start2)).h, (end = hcl(end)).h), c5 = nogamma(start2.c, end.c), l = nogamma(start2.l, end.l), opacity = nogamma(start2.opacity, end.opacity);
    return function(t) {
      start2.h = h(t);
      start2.c = c5(t);
      start2.l = l(t);
...
function styleMaybeRemove(id2, name) {
  ...
  return function() {
    var schedule = set3(this, id2), on = schedule.on, listener = schedule.value[key] == null ? remove || (remove = styleRemove(name)) : void 0;
...
function ribbon(headRadius) {
  ...
  function ribbon2() {
    var buffer, s = source.apply(this, arguments), t = target.apply(this, arguments), ap = padAngle.apply(this, arguments) / 2, argv = slice2.call(arguments), sr = +sourceRadius.apply(this, (argv[0] = s, argv)), sa0 = startAngle.apply(this, argv) - halfPi2, sa1 = endAngle.apply(this, argv) - halfPi2, tr = +targetRadius.apply(this, (argv[0] = t, argv)), ta0 = startAngle.apply(this, argv) - halfPi2, ta1 = endAngle.apply(this, argv) - halfPi2;
...
function contours_default() {
  ...
  function isorings(values, value, callback) {
    ...
    while (++x3 < dx - 1) {
      t03 = t13, t13 = above(values[x3 + 1], value);
      cases[t03 | t13 << 1].forEach(stitch);
    }
    cases[t13 << 0].forEach(stitch);
    while (++y3 < dy - 1) {
      x3 = -1;
      t13 = above(values[y3 * dx + dx], value);
      t22 = above(values[y3 * dx], value);
      cases[t13 << 1 | t22 << 2].forEach(stitch);
      while (++x3 < dx - 1) {
        t03 = t13, t13 = above(values[y3 * dx + dx + x3 + 1], value);
        t32 = t22, t22 = above(values[y3 * dx + x3 + 1], value);
        cases[t03 | t13 << 1 | t22 << 2 | t32 << 3].forEach(stitch);
      }
      cases[t13 | t22 << 3].forEach(stitch);
    }
    x3 = -1;
    t22 = values[y3 * dx] >= value;
...
function pad(value, width) {
  var s = value + "", length3 = s.length;
  return length3 < width ? new Array(width - length3 + 1).join(0) + s : s;
}
...
function linePoint(lambda, phi2) {
  ...
  if (p0) {
    ...
    var delta = lambda - lambda2, sign2 = delta > 0 ? 1 : -1, lambdai = inflection[0] * degrees3 * sign2, phii, antimeridian = abs3(delta) > 180;
...
function centroid_default(object2) {
  ...
  return [atan2(y3, x3) * degrees3, asin(z / m3) * degrees3];
}
...
function circleRadius(cosRadius, point2) {
  ...
  return ((-point2[2] < 0 ? -radius : radius) + tau4 - epsilon6) % tau4;
}
...
function circle_default3(radius) {
  var cr = cos2(radius), delta = 2 * radians2, smallRadius = cr > 0, notHemisphere = abs3(cr) > epsilon6;
...
function interpolate_default2(a3, b) {
  var x06 = a3[0] * radians2, y06 = a3[1] * radians2, x12 = b[0] * radians2, y12 = b[1] * radians2, cy0 = cos2(y06), sy0 = sin2(y06), cy1 = cos2(y12), sy1 = sin2(y12), kx0 = cy0 * cos2(x06), ky0 = cy0 * sin2(x06), kx1 = cy1 * cos2(x12), ky1 = cy1 * sin2(x12), d = 2 * asin(sqrt(haversin(y12 - y06) + cy0 * cy1 * haversin(x12 - x06))), k = sin2(d);
  var interpolate = d ? function(t) {
    var B3 = sin2(t *= d) / k, A5 = sin2(d - t) / k, x3 = A5 * kx0 + B3 * kx1, y3 = A5 * ky0 + B3 * ky1, z = A5 * sy0 + B3 * sy1;
    return [
      atan2(y3, x3) * degrees3,
      atan2(z, sqrt(x3 * x3 + y3 * y3)) * degrees3
    ];
  } : function() {
...
function resample(project, delta2) {
  function resampleLineTo(x06, y06, lambda04, a0, b02, c0, x12, y12, lambda12, a1, b12, c1, depth, stream) {
    ...
    if (d2 > 4 * delta2 && depth--) {
      var a3 = a0 + a1, b = b02 + b12, c5 = c0 + c1, m3 = sqrt(a3 * a3 + b * b + c5 * c5), phi2 = asin(c5 /= m3), lambda22 = abs3(abs3(c5) - 1) < epsilon6 || abs3(lambda04 - lambda12) < epsilon6 ? (lambda04 + lambda12) / 2 : atan2(b, a3), p = project(lambda22, phi2), x22 = p[0], y22 = p[1], dx2 = x22 - x06, dy2 = y22 - y06, dz = dy * dx2 - dx * dy2;
...
function projectionMutator(projectAt) {
  var project, k = 150, x3 = 480, y3 = 250, lambda = 0, phi2 = 0, deltaLambda = 0, deltaPhi = 0, deltaGamma = 0, rotate, alpha = 0, sx = 1, sy = 1, theta = null, preclip = antimeridian_default, x06 = null, y06, x12, y12, postclip = identity_default3, delta2 = 0.5, projectResample, projectTransform, projectRotateTransform, cache, cacheStream;
...
function albersUsa_default() {
  ...
  albersUsa.translate = function(_) {
    ...
    alaskaPoint = alaska.translate([x3 - 0.307 * k, y3 + 0.201 * k]).clipExtent([[x3 - 0.425 * k + epsilon6, y3 + 0.12 * k + epsilon6], [x3 - 0.214 * k - epsilon6, y3 + 0.234 * k - epsilon6]]).stream(pointStream);
    hawaiiPoint = hawaii.translate([x3 - 0.205 * k, y3 + 0.212 * k]).clipExtent([[x3 - 0.214 * k + epsilon6, y3 + 0.166 * k + epsilon6], [x3 - 0.115 * k - epsilon6, y3 + 0.234 * k - epsilon6]]).stream(pointStream);
    return reset();
  };
...
function azimuthalInvert(angle2) {
  return function(x3, y3) {
    var z = sqrt(x3 * x3 + y3 * y3), c5 = angle2(z), sc = sin2(c5), cc2 = cos2(c5);
    return [
      atan2(x3 * sc, z * cc2),
      asin(z && y3 * sc / z)
    ];
...
function conicConformalRaw(y06, y12) {
  ...
  function project(x3, y3) {
    if (f > 0) {
      if (y3 < -halfPi3 + epsilon6) y3 = -halfPi3 + epsilon6;
    } else {
      if (y3 > halfPi3 - epsilon6) y3 = halfPi3 - epsilon6;
    }
...
function transformer2() {
  ...
  function scale2(x3) {
    return x3 == null || isNaN(x3 = +x3) ? unknown : (output || (output = piecewise2(domain.map(transform2), range4, interpolate)))(transform2(clamp(x3)));
  }
...
function loggish(transform2) {
  ...
  scale2.tickFormat = (count3, specifier) => {
    ...
    const k = Math.max(1, base * count3 / scale2.ticks().length);
    return (d) => {
...
function formatLocale(locale3) {
  ...
  function newFormat(specifier, formats2) {
    return function(date2) {
      var string = [], i = -1, j = 0, n = specifier.length, c5, pad3, format2;
      if (!(date2 instanceof Date)) date2 = /* @__PURE__ */ new Date(+date2);
      while (++i < n) {
        if (specifier.charCodeAt(i) === 37) {
          string.push(specifier.slice(j, i));
          if ((pad3 = pads[c5 = specifier.charAt(++i)]) != null) c5 = specifier.charAt(++i);
          else pad3 = c5 === "e" ? " " : "0";
          if (format2 = formats2[c5]) c5 = format2(date2, pad3);
          string.push(c5);
          j = i + 1;
        }
      }
  ...
  function parseSpecifier(d, specifier, string, j) {
    var i = 0, n = specifier.length, m3 = string.length, c5, parse;
    while (i < n) {
      if (j >= m3) return -1;
      c5 = specifier.charCodeAt(i++);
      if (c5 === 37) {
        c5 = specifier.charAt(i++);
        parse = parses[c5 in pads ? specifier.charAt(i++) : c5];
...
function transformer3() {
  ...
  function scale2(x3) {
    return x3 == null || isNaN(x3 = +x3) ? unknown : interpolator(k10 === 0 ? 0.5 : (x3 = (transform2(x3) - t03) * k10, clamp ? Math.max(0, Math.min(1, x3)) : x3));
  }
  scale2.domain = function(_) {
...
function transformer4() {
  var x06 = 0, x12 = 0.5, x22 = 1, s = 1, t03, t13, t22, k10, k21, interpolator = identity3, transform2, clamp = false, unknown;
  function scale2(x3) {
    return isNaN(x3 = +x3) ? unknown : (x3 = 0.5 + ((x3 = +transform2(x3)) - t13) * (s * x3 < s * t13 ? k10 : k21), interpolator(clamp ? Math.max(0, Math.min(1, x3)) : x3));
  }
  scale2.domain = function(_) {
    return arguments.length ? ([x06, x12, x22] = _, t03 = transform2(x06 = +x06), t13 = transform2(x12 = +x12), t22 = transform2(x22 = +x22), k10 = t03 === t13 ? 0 : 0.5 / (t13 - t03), k21 = t13 === t22 ? 0 : 0.5 / (t22 - t13), s = t13 < t03 ? -1 : 1, scale2) : [x06, x12, x22];
  };
  scale2.clamp = function(_) {
    return arguments.length ? (clamp = !!_, scale2) : clamp;
  };
  ...
  return function(t) {
    transform2 = t, t03 = t(x06), t13 = t(x12), t22 = t(x22), k10 = t03 === t13 ? 0 : 0.5 / (t13 - t03), k21 = t13 === t22 ? 0 : 0.5 / (t22 - t13), s = t13 < t03 ? -1 : 1;
    return scale2;
  };
}
function diverging() {
...
function zoom_default2() {
  ...
  zoom.scaleTo = function(selection, k, p, event) {
    zoom.transform(selection, function() {
      var e = extent2.apply(this, arguments), t03 = this.__zoom, p02 = p == null ? centroid(e) : typeof p === "function" ? p.apply(this, arguments) : p, p1 = t03.invert(p02), k1 = typeof k === "function" ? k.apply(this, arguments) : k;
  ...
  function dblclicked(event, ...args) {
    ...
    noevent_default3(event);
    if (duration > 0) select_default(this).transition().duration(duration).call(schedule, t13, p02, event);
    else select_default(this).call(zoom.transform, t13, p02, event);
  }
